...
export class ContactlistComponent implements OnInit {
  ...
  Add() {
    if(this.nform?.value){
      if(!this.Editmode){
        ...
        this.http.post('http://phonebook9.herokuapp.com/api/add',data).subscribe(res=>{
        ...
        this.http.patch("http://phonebook9.herokuapp.com/api/update/"+data,this.nform.value).subscribe(dat=>{
  ...
  Delete(){
    let data=this.PhoneList[this.indexnumber]._id
    this.http.delete("http://phonebook9.herokuapp.com/delete/"+data).subscribe()
